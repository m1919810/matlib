...
//     * @return
//     */
//    public static void clearAmount(BlockMenu inv, ItemWithSlot... counters) {
//        ItemWithSlot ip;
...
//     */
//    public static ItemConsumer[] matchRecipe(List<ItemWithSlot> slotCounters, MachineRecipe recipe) {
...
//        List<ItemWithSlot> slotCounters, MachineRecipe recipe, int maxMatchCount) {
//        int len2 = slotCounters.size();
//        ItemStack[] recipeInput = recipe.getInput();
//        int cnt = recipeInput.length;
//        if (cnt > len2) return null;
...
//            for (int j = 0; j < len2; ++j) {
//                ItemWithSlot itemCounter2 = slotCounters.get(j);
//                if (itemCounter2 == null) continue;
//                if (!visited[j]) {
//                    itemCounter2.syncData();
//                    visited[j] = true;
...
//        int cnt = recipeInput.length;
//        ItemConsumer[] result = new ItemConsumer[cnt];
//        LazyArray<ItemWithSlot> slotCounters =
//                new LazyArray<>(ItemWithSlot[]::new, len2, pusher.getMenuInstance(Flags.OUTPUT, inv, output));
//        for (int i = 0; i < cnt; ++i) {
//            result[i] = getConsumer(recipeInput[i]);
//            for (int j = 0; j < len2; ++j) {
...
//        LazyArray<ItemWithSlot> inputs =
//                new LazyArray<>(ItemWithSlot[]::new, len, pusher.getMenuInstance(Flags.INPUT, inv, input));
//
...
//    }
//    /**
//     * match max craft time of recipe ,return null if cannot craft return the recorded information of inputConsumer
// and outputConsumner
//     * @param inv
//     * @param input
//     * @param output
...
//        int len = input.length;
//        ItemStack[] recipeInput = recipe.getInput();
//        LazyArray<ItemWithSlot> inputCounters =
//                new LazyArray<>(ItemWithSlot[]::new, len, pusher.getMenuInstance(Flags.INPUT, inv, input));
//        int cnt = recipeInput.length;
//
...
//            for (int j = 0; j < len; ++j) {
//                ItemWithSlot itemCounter2 = inputCounters.get(j);
//                if (itemCounter2 == null) continue;
//                if (!visited[j]) {
//                    itemCounter2.syncData();
//                    visited[j] = true;
...
//
//    /**
//     * return null if no match ,return ItemGreedyConsumer with modification of inputInfo and outputConsumer with
// written matchAMount
//     * @param inputInfo
//     * @param inv
//     * @param output
...
//
//        int len2 = output.length;
//        LazyArray<ItemWithSlot> outputCounters =
//                new LazyArray<>(ItemWithSlot[]::new, len2, pusher.getMenuInstance(Flags.OUTPUT, inv, output));
...
//                ItemWithSlot itemCounter = outputCounters.get(i);
...
//                for (int j = 0; j < len2; ++j) {
//                    ItemWithSlot itemCounter = outputCounters.get(j);
//                    if (itemCounter.getItem() == null) {
...
//
//    /**
//     * this method do not have step reading optimiz,should provide pre-get ItemPushers,used in Places where outputslot
...
//    @Deprecated
//    public static ItemGreedyConsumer[] countMultiOutput(
//        ItemGreedyConsumer[] inputInfo, ItemWithSlot[] output, MachineRecipe recipe, int limit) {
//
//        int len2 = output.length;
...
//                ItemWithSlot itemCounter = output[outputSlotpointer];
//                ++outputSlotpointer;
//                if (itemCounter.getItem() == null) {
//                    itemCounter.setFrom(recipeCounter2[0]);
//                    recipeCounter2[0].addRelate(itemCounter);
...
//                boolean hasNextPushSlot = false;
//                for (int j = 0; j < len2; ++j) {
//                    ItemWithSlot itemCounter = output[j];
...
//
//    /**
//     * public static Pair<ItemGreedyConsumer[],ItemGreedyConsumer[]> countMultiRecipe(List<ItemPusher> inputCounters,
// ItemSlotPusher[] outputCounters, MachineRecipe recipe, int limit){
...
//        LazyArray<ItemWithSlot> slotCounters2 =
//                new LazyArray<>(ItemWithSlot[]::new, slots.length, pusher.getMenuInstance(Flags.OUTPUT, inv, slots));
//        ItemConsumer outputItem;
//        ItemWithSlot itemCounter;
...
//                            continue;
//                        } else if (CraftUtils.matchItemCounter(outputItem, itemCounter, false)) {
//                            itemCounter.grab(outputItem);
//                            itemCounter.updateMenu(inv);
//                            itemCounter.setDirty(true);
//                            hasChanged = true;
...
//                            continue;
//                        } else if (CraftUtils.matchItemCounter(outputItem, itp, false)) {
//                            outputItem.push(itp);
//                            itp.updateMenu(inv);
//                            itp.setDirty(true);
//                            hasChanged = true;
...
//     * @param inv
//     */
//    public static void updateOutputMenu(ItemWithSlot[] itemCounters, int[] out, BlockMenu inv) {
//        int len = out.length;
//        for (int i = 0; i < len; ++i) {
//            ItemWithSlot a = itemCounters[i];
//            if (a == null) {
//                continue;
//            }
//            if (a.isDirty()) {
...
//        final LazyArray<ItemWithSlot> slotCounter =
//                new LazyArray<>(ItemWithSlot[]::new, len, pusher.getMenuInstance(Flags.INPUT, inv, slots));
...
//
//    /**
//     * general only find next recipe for input slots ,No check for output slots, no consume for inputslots ,only fetch
// NEXT matching recipe.
...
//            }
//        }
//        LazyArray<ItemWithSlot> slotCounter = new LazyArray<>(
//                ItemWithSlot[]::new, slotNotNull.size(), pusher.getMenuInstance(Flags.INPUT, inv, slotNotNull));
//        int recipeAmount = recipes.size();
//        if (recipeAmount <= 0) {
//            return null;
//        }
...
//     */
//    /**
//     * public static Pair<MachineRecipe,ItemGreedyConsumer[]> findNextMultiRecipe(BlockMenu inv,int[] inputs,int[]
// outputs,
...
//     * }
//     **/
//    public static int matchShapedRecipe(ItemWithSlot[] input, MachineRecipe recipe, int limit) {
//        ItemStack[] recipeInput = recipe.getInput();
//        int len = input.length;
...
//        }
//        int len = inputs.length;
//        ItemWithSlot[] inputItem = new ItemWithSlot[len];
//        IntFunction<ItemWithSlot> inputSlotInstance = pusher.getMenuInstance(Flags.INPUT, inv, inputs);
//        for (int i = 0; i < len; ++i) {
...
//    }
//
//    public static boolean matchSequenceRecipeTarget(ItemWithSlot[] inPushers, ItemConsumer target) {
...
//                continue;
//            } else if (CraftUtils.matchItemCounter(target, inPushers[i], false)) {
//                hasChange = true;
...
//        }
//        int len = inputs.length;
//        ItemWithSlot[] inputCounters = new ItemWithSlot[len];
//        IntFunction<ItemWithSlot> pusherFunc = pusher.getMenuInstance(Flags.INPUT, inv, inputs);
//        for (int i = 0; i < len; ++i) {
