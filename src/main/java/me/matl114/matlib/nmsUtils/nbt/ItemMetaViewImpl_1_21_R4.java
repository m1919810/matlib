...
class ItemMetaViewImpl_1_21_R4 extends ItemMetaViewImpl_1_20_R4 {
    ...
    protected void setFlag(ItemFlag flag, boolean shouldDisplay){
        ...
        if(tooltips != null){
            //already have flags
            SequencedSet<?> hidden = DATA_TYPES.tooltipsDisplay$hiddenComponents(tooltips);
            if(hidden.containsAll(hides) == !shouldDisplay){
                ...
                boolean hideFlag = DATA_TYPES.tooltipsDisplay$hideTooltips(tooltips);
                if(!newSet.isEmpty() || hideFlag){
                    HELPER.setDataComponentValue(itemStack, DataComponentEnum.TOOLTIP_DISPLAY, DATA_TYPES.tooltipsDisplay$newComponent(hideFlag, newSet));
                }else{
            ...
            if (!shouldDisplay){
                //do not display, so
                //add these Flag
                HELPER.setDataComponentValue(itemStack, DataComponentEnum.TOOLTIP_DISPLAY, DATA_TYPES.tooltipsDisplay$newComponent(false, new ReferenceLinkedOpenHashSet<>(hides)));
            }
        }
    ...
    protected boolean getVisibilityForFlag(ItemFlag flag){
        ...
        return !DATA_TYPES.tooltipsDisplay$hiddenComponents(tooltips).containsAll(ITEM_FLAG_EQUIVALENTS.getOrDefault(flag, Set.of()));
    }
    public boolean isHideTooltip(){
        ...
        return DATA_TYPES.tooltipsDisplay$hideTooltips(tooltips);
    }
    public void setHideTooltip(boolean b){
        Object tooltips = HELPER.getFromPatch(itemStack, DataComponentEnum.TOOLTIP_DISPLAY);
        if(tooltips != null){
            SequencedSet<?> set = DATA_TYPES.tooltipsDisplay$hiddenComponents(tooltips);
            if(!set.isEmpty() || b){
                HELPER.setDataComponentValue(itemStack, DataComponentEnum.TOOLTIP_DISPLAY, DATA_TYPES.tooltipsDisplay$newComponent(b, set));
            }else{
            ...
            if(b){
                HELPER.setDataComponentValue(itemStack, DataComponentEnum.TOOLTIP_DISPLAY, DATA_TYPES.tooltipsDisplay$newComponent(true, new ReferenceLinkedOpenHashSet<>()));
            }
        }
    }
}
