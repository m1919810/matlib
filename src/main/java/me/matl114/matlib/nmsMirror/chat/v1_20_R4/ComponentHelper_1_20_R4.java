...
public interface ComponentHelper_1_20_R4 extends ComponentHelper {
    ...
    default String toJson(@RedirectType(ChatComponent)Iterable<?> comp,@RedirectType("Lnet/minecraft/core/HolderLookup$Provider;") Object registry){
        return GSON.toJson(serialize(comp, registry));
    }
    @MethodTarget(isStatic = true)
    @RedirectClass(ChatComponentSerializer)
    @IgnoreFailure(thresholdInclude = Version.v1_21_R5)
    default Iterable<?> deserialize(JsonElement json, @RedirectType("Lnet/minecraft/core/HolderLookup$Provider;") Object registry){
        return DataHelper.A.I.getOrThrow(CodecEnum.CHAT_COMPONENT.parse(REGISTRIES.provideRegistryForDynamicOp(registry, JsonOps.INSTANCE), json), JsonParseException::new);
    }

    @MethodTarget(isStatic = true)
    @RedirectClass(ChatComponentSerializer)
    @IgnoreFailure(thresholdInclude = Version.v1_21_R5)
    default JsonElement serialize(@RedirectType(ChatComponent)Iterable<?> comp, @RedirectType("Lnet/minecraft/core/HolderLookup$Provider;") Object registry){
        return DataHelper.A.I.getOrThrow(CodecEnum.CHAT_COMPONENT.encodeStart(REGISTRIES.provideRegistryForDynamicOp(registry, JsonOps.INSTANCE), comp), JsonParseException::new);
    }

    @MethodTarget(isStatic = true)
    @RedirectClass(ChatComponentSerializer)
    @IgnoreFailure(thresholdInclude = Version.v1_21_R5)
    default Iterable<?> fromJson(String json, @RedirectType("Lnet/minecraft/core/HolderLookup$Provider;") Object registry){
    ...
    @RedirectClass(ChatComponentSerializer)
    @IgnoreFailure(thresholdInclude = Version.v1_21_R5)
    default Iterable<?> fromJson(JsonElement json, @RedirectType("Lnet/minecraft/core/HolderLookup$Provider;") Object registry){
