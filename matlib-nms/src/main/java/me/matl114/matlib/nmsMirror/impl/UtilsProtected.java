...
class UtilsProtected {
    ...
    static <T> Codec<T> fcVer(String value, Version version, String... val) {
        ...
        return !Version.getVersionInstance().isAtLeast(version)
                ? Utils.matchNull(fields, val[0])
                : Utils.matchName(fields, val[0]);
    }
    ...
    static Codec<Object> g1Ver(Version version) {
        return !Version.getVersionInstance().isAtLeast(version)
                ? SHARING_OBJECT_1.get()
                : Objects.requireNonNull(SHARING_OBJECT_1.get());
    }
    ...
    static Codec<Object> g2Ver(Version version) {
        return !Version.getVersionInstance().isAtLeast(version)
                ? SHARING_OBJECT_2.get()
                : Objects.requireNonNull(SHARING_OBJECT_2.get());
    }
}
