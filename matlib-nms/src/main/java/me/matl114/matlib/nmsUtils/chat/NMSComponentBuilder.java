...
public class NMSComponentBuilder extends ComponentVisitor {
    ...
    public void visitClickEvent(ClickEvent.Action clickEvent, List rawData) {
        ...
        if (isConst) {
            ...
            Object eventConst = NMSChat.FORMAT.newClickEvent(typeNMS, value);
            if (constEvent) {
            ...
            eventProcessor.add((arg, sty) -> {
                String value = (String) rawData.stream()
                        ...
                        .collect(Collectors.joining());
                sty.clickEvent(NMSChat.FORMAT.newClickEvent(typeNMS, value));
            });
        }
    }
    ...
    public void visitHoverEvent(HoverEvent.Action hoverEvent, Object rawData) {
        ...
        if (hoverEvent == HoverEvent.Action.SHOW_TEXT) {
            if (NMSChat.CHATCOMPONENT.isComponent(rawData)) {
                Object hoverEvent0 = NMSChat.FORMAT.createHoverEventByRaw(HoverEvent.Action.SHOW_TEXT, rawData);
                if (constEvent) {
                ...
                Object hoverEvent0 = NMSChat.FORMAT.createHoverEventByRaw(
                ...
                eventProcessor.add((arg, sty) -> {
                    ...
                    if (NMSChat.CHATCOMPONENT.isComponent(optionalComp)) {
                        sty.hoverEvent(NMSChat.FORMAT.createHoverEventByRaw(HoverEvent.Action.SHOW_TEXT, optionalComp));
                    } else if (optionalComp instanceof MutableBuilder builder) {
                        Object comp = builder.toNMS();
                        Object hoverEvent0 = NMSChat.FORMAT.createHoverEventByRaw(HoverEvent.Action.SHOW_TEXT, comp);
            ...
            if (rawData instanceof ItemStack stack) {
                ...
                Object hoverEventConst = NMSChat.FORMAT.createHoverEventByRaw(HoverEvent.Action.SHOW_ITEM, nms);
                if (constEvent) {
                ...
                eventProcessor.add((arg, sty) -> {
                    ...
                    if (stack != null) {
                        Object nms = ItemUtils.unwrapHandle(stack);
                        Object hoverEvent0 = NMSChat.FORMAT.createHoverEventByRaw(HoverEvent.Action.SHOW_ITEM, nms);
                        sty.hoverEvent(hoverEvent0);
                    }
                });
                this.constEvent = false;
    ...
    public static ContentBuilder initBuilder(ComponentContentType type) {
        return switch (type) {
            case ComponentContentType.TRANSLATABLE -> new TranslatableContentBuilder();
            case ComponentContentType.LITERAL -> new LiteralContentBuilder();
            case ComponentContentType.EMPTY -> null;
            case ComponentContentType.NBT -> throw new NotImplementedYet();
                // not impl yet
            case ComponentContentType.SCORE -> new LiteralContentBuilder();
            case ComponentContentType.KEYBIND -> new LiteralContentBuilder();
            case ComponentContentType.SELECTOR -> new LiteralContentBuilder();
        };
    }
                ...
                if (styleDynamicArray.length == 0) {
                    ...
                    if (builderDynamicArray.length == 0) {
                        ...
                        finalFunction = (args) -> {
                            ...
                            NMSChat.CHATCOMPONENT.setStyle(valueNoStyle, newStyle.toNMS());
