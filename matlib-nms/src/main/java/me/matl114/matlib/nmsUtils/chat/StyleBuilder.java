...
public class StyleBuilder implements Cloneable, BuildResult<Object> {
    ...
    public Object toNMS() {
        return NMSChat.FORMAT.newStyle(
                this.color,
                this.bold,
                this.italic,
                this.underlined,
    ...
    public StyleBuilder withColor(int rgb) {
        return color(NMSChat.FORMAT.textcolorFromRgb(rgb));
    }
    ...
    public StyleBuilder applyFormatFrom(Object style) {
        this.color = NMSChat.FORMAT.getColor(style);
        this.bold = NMSChat.FORMAT.isBold(style);
        this.italic = NMSChat.FORMAT.isItalic(style);
        this.underlined = NMSChat.FORMAT.isUnderlined(style);
        this.strikethrough = NMSChat.FORMAT.isStrikethrough(style);
        this.obfuscated = NMSChat.FORMAT.isObfuscated(style);
        return this;
    }

    public StyleBuilder applyEventFrom(Object style) {
        this.clickEvent = NMSChat.FORMAT.getClickEvent(style);
        this.hoverEvent = NMSChat.FORMAT.getHoverEvent(style);
        this.font = NMSChat.FORMAT.getFont(style);
        this.insertion = NMSChat.FORMAT.getInsertion(style);
        return this;
    }
