...
public class PlayerUtils {
    public static Collection<Entity> getViewableEntity(Player player) {
        Object nms = CraftBukkit.ENTITY.getHandle(player);
        Object conn = NMSLevel.PLAYER.connectionGetter(nms);
        World world = player.getWorld();
        Object worldHandle = WorldUtils.getHandledWorld(world);
        Object chunkCache = NMSLevel.LEVEL.getChunkSource(worldHandle);
        Object entityMap = NMSLevel.CHUNK_CACHE_SYSTEM.chunkMapGetter(chunkCache);
        Int2ObjectMap<?> entityMap0 = NMSLevel.CHUNK_MAP.entityMapGetter(entityMap);
        Set<Entity> entities = new HashSet<>();
        for (var entry : entityMap0.int2ObjectEntrySet()) {
            Object tracker = entry.getValue();
            ReferenceOpenHashSet<?> seen = NMSLevel.CHUNK_MAP.seenByGetter(tracker);
            if (seen.contains(conn)) {
                // in view range
                Object entity = NMSLevel.CHUNK_MAP.entityGetter(tracker);
