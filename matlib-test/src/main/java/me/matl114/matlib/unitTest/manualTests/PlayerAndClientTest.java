        ...
        public void onServerboundInteract(PacketEvent packet) {
            ...
            if (user instanceof Player pl) {
                ...
                ThreadUtils.executeSync(() -> {
                    Object target = NMSNetwork.PACKETS.serverboundInteractPacket$getTarget(
                            packet.getPacket(), WorldUtils.getHandledWorld(world));
                    ...
                    if (targetEntity instanceof Player player) {
                        ...
                        if (dis > MathUtils.square(interactRange + 1.0F)) {
                            component = ComponentUtils.fromLegacyString(
                                    AddUtils.resolveColor("&c%s 在试图攻击 %s 的时候失败了, 距离 %.2f"
                                            .formatted(pl.getName(), player.getName(), Math.sqrt(dis))));

                        } else {
                            component = ComponentUtils.fromLegacyString(AddUtils.resolveColor(
                                    "&a%s 成功攻击 %s ,距离 %.2f".formatted(pl.getName(), player.getName(), Math.sqrt(dis))));
                        }
                        Location loc2 = pl.getLocation();
                        Location loc3 = player.getLocation();
                        String cmp = "[%.2f,%.2f,%.2f]".formatted(loc2.getX(), loc2.getY(), loc2.getZ())
                                + "[%.2f,%.2f,%.2f]".formatted(loc3.getX(), loc3.getY(), loc3.getZ());
                        component = component.style(component.style().toBuilder()
                                .hoverEvent(ComponentUtils.fromLegacyString(AddUtils.resolveColor("&c双方位置: 攻击方: "
                                        + "[%.2f,%.2f,%.2f]".formatted(loc2.getX(), loc2.getY(), loc2.getZ())
                                        + ", 被攻击方 : "
                                        + "[%.2f,%.2f,%.2f], 点击拷贝".formatted(loc3.getX(), loc3.getY(), loc3.getZ()))))
