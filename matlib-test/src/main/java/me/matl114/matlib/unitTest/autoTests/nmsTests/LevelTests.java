...
public class LevelTests implements TestCase {
    ...
    public void test_worldAndBlockEntity() throws Throwable {
        ...
        var chunk1 = NMSLevel.LEVEL.getChunkCustom(serverLevel, cx, cz, true);
        AssertNN(chunk1);
        Debug.logger(chunk1);
        var entity1 = LevelUtils.getBlockEntityAsync(block, false);
        AssertNN(entity1);
        Debug.logger(NMSLevel.BLOCK_ENTITY.saveWithFullMetadata(entity1));
        Debug.logger(NMSLevel.BLOCK_ENTITY.saveWithId(entity1));
        Object pdc = NMSLevel.BLOCK_ENTITY.getPersistentDataCompound(entity1);
        NMSCore.COMPOUND_TAG.clear(pdc);
        NMSCore.COMPOUND_TAG.putBoolean(pdc, "testBoolean", true);
        Debug.logger(NMSLevel.BLOCK_ENTITY.saveWithFullMetadata(entity1));
